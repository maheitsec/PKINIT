                       ...
                       krb5_keyblock *key_block)
{
    ...
    if (NULL == (key_block->contents = (krb5_octet *)malloc(key_block->length))) {
	//End
        retval = ENOMEM;
        goto cleanup;
    }
    ...
    if (NULL == (random_data.data = (char *)malloc(reps * hash_len))) {
	//End
        retval = ENOMEM;
        goto cleanup;
    }
                 ...
                 unsigned int *dh_pubkey_len)
{
    ...
    *dh_pubkey_len = i2d_ASN1_INTEGER(pub_key, NULL);
    ...
    if ((buf = *dh_pubkey = (unsigned char *)malloc(*dh_pubkey_len)) == NULL) {
	//End
        retval  = ENOMEM;
        goto cleanup;
    }
                  ...
                  unsigned int *client_key_len)
{
    ...
    *client_key_len = DH_size(cryptoctx->dh);
    ...
    if ((*client_key = (unsigned char *)malloc(*client_key_len)) == NULL) {
	//End
        retval = ENOMEM;
        goto cleanup;
    }
